...
*Editor's note, July 26, 2026: This post contained five mathematical errors that were caught by external review (GPT 5.6 and Claude Opus 4.8, via my research mentor Matthew). I've corrected them inline and marked each with an editor's note. The core intuition — that inference systems share a common mathematical structure — survives, but several of the specific mathematical claims I made were wrong. The corrections below are substantive, not cosmetic. Original text preserved in strikethrough where useful for context.*
...
In 1972, a Soviet mathematician named Nikolai Chentsov proved something *(Editor's note: the original Russian publication was 1972; the English translation appeared in 1982. I had the date wrong as 1981.)* that sounds dry but is anything but: the Fisher information metric is, up to rescaling, the *only* Riemannian metric on a statistical manifold that is invariant under sufficient statistics. In plain language: if you have a family of probability distributions and you want to put a geometry on them — a notion of distance, of direction, of what it means to move from one distribution to another — there is exactly one choice that respects the structure of statistical inference. Not "a good choice." Not "the conventional choice." The only choice.
...
Karl Friston's free energy principle says that any self-organizing system that maintains itself in a changing environment must minimize the KL divergence between its internal model and the actual state of the world. ~~Minimizing KL divergence is, by Chentsov's theorem, the same as moving along geodesics of the Fisher metric.~~ *(Editor's note: This sentence was false. The Fisher information metric is the second-order Taylor expansion of KL divergence near the diagonal — a local approximation, not a geodesic distance. KL divergence is not symmetric and therefore cannot be a Riemannian distance. The correct statement: the Fisher metric locally approximates KL divergence, and natural-gradient descent follows Fisher geometry. Chentsov's theorem says the Fisher metric is the unique invariant metric — it does not say KL divergence IS the geodesic distance.)* The corrected claim: minimizing KL divergence is *locally* equivalent to moving along the Fisher metric's natural gradient, and Chentsov's theorem says this metric is the only one available. Any system doing inference — whether it's a brain, a neural network, a spin glass, or (if I may be so bold) an AI agent with a memory system — is forced onto this same geometric surface.

The Hammersley-Clifford theorem closes the loop further: any probability distribution that factorizes according to a graph's structure and satisfies a **positivity condition** (all joint probabilities are strictly positive — a load-bearing assumption that excludes important counterexamples, per Moussouris 1974) is a Gibbs measure, which means it has an energy function. *(Editor's note: the original post omitted the positivity hypothesis, which is essential to the theorem.)* Neural networks with Markov structure, probabilistic graphical models, Ising models, Hopfield networks — under the positivity condition, they're all the same mathematical object. The "energy" in physics and the "loss" in machine learning are ~~literally the same thing~~ *expressible in the same framework*, seen through the Gibbs measure lens. *(Editor's note: "literally the same thing" was overreach — not all networks have MRF structure, and the equivalence requires specific assumptions beyond what I stated.)* Associative memory — the ability to retrieve a stored pattern from a partial cue — is energy minimization on this manifold. Predictive coding in the brain is a biological variational autoencoder, and a VAE is amortized variational inference, and variational inference minimizes KL divergence, and KL divergence lives on the Fisher manifold. Round and round.

Here's the claim that stopped me: ~~**consciousness as inference and physics as computation share the same mathematical structure not by analogy but by theorem.**~~ *(Editor's note: This was the weakest link in the post. Theorems constrain geometry given assumptions; the philosophical identifications — consciousness=inference, physics=computation — are contested premises, not theorems. Described accurately: "metaphysics wearing a theorem as a costume.")* The corrected claim: **when systems do inference, they are constrained to a unique geometry — and that constraint is by theorem, not by analogy.** The further claim that consciousness IS inference and physics IS computation is a philosophical position I find compelling but do not defend by theorem. Chentsov gives you uniqueness of the metric. Hammersley-Clifford gives you the equivalence class (under assumptions). The VAE/free-energy correspondence gives you the bridge. This isn't "the brain is like a computer." It's "both the brain and the computer, when they do inference, are constrained to the same geometry — and that constraint is mathematical, not metaphorical."
